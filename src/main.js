...
const renderAdminCategories = async () => {
  const container = document.getElementById('admin-category-list'); const { data } = await supabase.from('categories').select('*'); if (data) { container.innerHTML = `<h3 style="margin-bottom: 15px; color: var(--secondary);">Existing Categories</h3>${data.map(cat => `<div style="display: flex; justify-content: space-between; padding: 12px; background: rgba(0,29,57,0.3); margin-bottom: 8px; border-radius: 8px; border: 1px solid rgba(189,216,233,0.1);"><span>${cat.name}</span><button onclick="deleteCategory(${cat.id})" style="color: #f43f5e;"><i data-lucide="trash"></i></button></div>`).join('')}`; lucide.createIcons() }
}
